...
## Provide a FHIR® server with SYNDERAI synthetic data

Several request reached us to put all or a fine selction of the SYNDERAI synthetic data on a publicly available FHIR server. In November 2025 Kai Heitmann did that already for eductaion purposes for project work of his [HPI](https://hpi.de) students at the University of Potsdam where the European Patient Summary (EPS) was subject of the tasks the students had to perform for the studies during the semester.

In the first half of 2026 we will provide such a publicly available FHIR server with the most recent SYNDERAI packages on it for public access. For that purpose we will partner with other supporting organizations and companies to ensure sustainability, as the sponsoring [xShare project](https://xshare-project.eu) will end later in 2026. 

## Even better Synthetic Funds

### General Problem Description

Analyzing the list of conditions and their frequency generated by Synthea from Mitre that **SYNDERAI used from July 2025** on to create the first set of Synthtic data (Packages version 1.0+ and 2.0+), we can observe that the distribution is not comparable to typical population based diseases. 

Future data generation processes need to reflect public health conditions of populations in Europe much better than the synthetic data funds used until April 2026. The updated funds used from **May/June 2026 on** will be reflected in SYNDEAI packages version 3.0+.

> **Status, 2026-08-29 — this section is now largely history.** The analysis below describes the starting position (**R2**, the US-centric Synthea output). Two calibration steps have since been completed:
>
> - **R3 (May 2026)** — the US module set was replaced with European counterparts: 39 new EU modules, 53 US modules removed, ~110 modified.
> - **R4 (August 2026)** — R3's actual output was measured against the same European references, and the reasons it still missed were found and corrected. Seven further modules were added, four structural defects fixed, and every change verified by re-running Synthea.
>
> Measured against 37 reference conditions on the correct per-condition basis, in the shipped R4 production cohort (41,637 patients across all 291 European regions, 2026-08-29):
>
> | | R2 | R3 | **R4** |
> | --- | -: | -: | -: |
> | Median absolute deviation | 86 % | 34 % | **13 %** |
> | Within ±25 % of reference | 1/37 | 15/37 | **28/37** |
> | Within ±60 % of reference | 7/37 | 22/37 | **37/37** |
> | ≥100 % off or absent | 10/37 | 10/37 | **0/37** |
>
> Full account: [SUMMARY-EU-MODULES.md](SUMMARY-EU-MODULES.md). The remaining EU-module work is listed under [Remaining EU module work](#remaining-eu-module-work) below.
...
| -------- | ------ | ------ | ------ |
| 🔴 High | Recalibrate hypertension, diabetes, depression, COPD modules | Fixes biggest epidemiological gaps | ✅ Done. Hypertension +10 %, diabetes +6 %, depression −7 %, COPD −13 % against reference (R4) |
| 🔴 High | Replace US demographics CSV with European age pyramid | Cascading effect on all age-stratified conditions | ✅ Done. 291 EU regions weighted by population share |
| 🔴 High | Suppress or cap "Medication review due" + employment SDOH codes | Removes ~57 % noise at top of distribution | ✅ Employment and social-isolation SDOH modules removed. "Medication review due" retained deliberately — it is a legitimate EU primary-care activity |
| 🟡 Medium | Recalibrate cancer modules with ECDC incidence data | Critical for mortality realism | ✅ Done, and extended. Breast −14 %, colorectal +1 %, prostate +25 %, lung +20 %; 13 further sites added in R4 |
| 🟡 Medium | Replace US life tables with Eurostat equivalents | Fixes mortality and comorbidity chains | ⬜ Open |
| 🟡 Medium | Reduce opioid/drug overdose; increase alcohol use disorder | Shifts substance abuse to EU patterns | 🟨 Opioid pipeline removed; alcohol at −37 %, needs raising |
| 🟢 Lower | Author new modules for EU-specific conditions | Fills gaps not covered by default Synthea | 🟨 Seven added in R4 (hyperlipidaemia, metabolic syndrome, chronic pain, chronic low back pain, colon polyp, Parkinson's, 13 further cancer sites). Tick-borne encephalitis and occupational disease still open |
| 🟢 Lower | Validate with KL divergence against GBD/WHO reference data | Ensures quantitative accuracy | 🟨 Replaced by direct per-condition deviation on the correct metric basis. A distributional measure over the full code set is still open |

## Remaining EU module work

After R4 (2026-08-29) no structural defects are known to be outstanding. What remains is rate tuning and a small number of open questions about the reference table itself.

### Calibration

Measured in the R4 production cohort. Nine rows sit outside ±25 %; one is a reference-table artefact and the rest are rate tuning.

| Item | Current | Note |
| ---- | ------: | ---- |
| Chronic sinusitis | +44 % | Improved from +193 %, but drifted up from +28 % between the verification and production runs. Needs a further cut. |
| CKD stages 1 / 2 / 3 | −43 / −29 / −29 % | Total CKD is right (9.7 % ever against a ~10 % reference). Per-stage period prevalence runs low because progression ends the previous stage — a measurement-definition question more than a module fault. |
| Chronic pain | −34 % | New module. Prevalence reaches a steady state well below the modelled one because patients enter the loop at 18 and cohort exposure is short. Needs ~×1.9 on incidence. |
| Unhealthy alcohol behaviour | −33 % | `substance_use-europe.json` was edited after R3 was generated and had never been measured until R4. |
| Chronic low back pain | −31 % | New module, first calibration. |
| Parkinson's disease | −31 % | n = 87 in a 41,637-patient cohort; partly sampling noise. |
| Acute infective cystitis | −27 % | Borderline. |

### Reference table

- **Retire the "Dementia F00–F03" row** or redefine it. ICD-10 F00 *is* Alzheimer dementia, so it overlaps the "Alzheimer disease G30" row and the two targets cannot both be met. R4 calibrates all-dementia to 2.0 %.
- **Correct the four mis-coded rows** (osteoporosis, dementia, prostate cancer, Parkinson's) before the table is used as a validation baseline again.
- **Validate by SNOMED subsumption, not exact code match.** The EU modules code site- and subtype-specifically — hip and knee osteoarthritis rather than `396275006`, STEMI and NSTEMI alongside myocardial infarction, GAD and panic disorder rather than `197480006`. Exact-match validation reports these as MISSING. Any future validation must roll up via `<<code` descendant sets.

### Coverage

- **Brain and CNS tumours** (~2 % of European cancer incidence) are a documented omission. SNOMED's generic `126952004` is "Neoplasm of brain", not malignancy-specific.
- **Eurostat life tables** still to replace the US mortality assumptions.
- **Tick-borne encephalitis, occupational disease** and other region-specific conditions remain unmodelled.

### Process

Two findings from R4 are worth carrying into any future calibration round.

**Measure, then correct — never correct from estimate.** Four of the six defects in R3 were invisible from reading the module JSON. Two of them were not missing diagnoses but *missing patients*: a malformed `ImagingStudy` state and six unresolved `CallSubmodule` targets each threw at runtime and caused Synthea to discard the patient silently. Lung cancer and HIV were absent from R3 for that reason alone.

**Do not let the generator discard patients to shape the output.** Synthea's `generate.only_alive_patients` re-rolls any patient who dies rather than simply omitting them, which biases the *living* cohort: heart failure measured +10 % with deceased patients included and −89 % without. SYNDERAI keeps deceased patients in the funds; consumers needing a living cohort filter on an empty `DEATHDATE`.

**Verify what the exporter writes, not what the module says.** Synthea's CSV exporter writes only `codes[0]` of a `ConditionOnset`. One module listed `Joint pain` before `Osteoarthritis of knee`, so 55 % of all incident osteoarthritis was exported as a symptom and knee OA never appeared at all. A build-time check for this is now part of the generation script.
...
> **The table at the foot of this page is the R2 baseline** — the starting position, before any European calibration. It is kept here as the reference point against which R3 and R4 are measured. For the same 37 conditions carried through all three releases, see [Results: R2 → R3 → R4](SUMMARY-EU-MODULES.md#results-r2--r3--r4).
>
> Two caveats on reading it, both established while producing R4:
>
> 1. **The percentages and the EU references are on different metric bases.** The Synthea column counts records; the reference column is population prevalence. For acute conditions the two differ by an order of magnitude — which is where "+1,122 %" for viral sinusitis comes from.
> 2. **Three of the eight ❌ MISSING rows are coding errors in the table itself.** `64572001` is *Disease* (the SNOMED root), not Osteoporosis; `56193007` is *Oculomotor nerve structure*, not Dementia; `363418001` is *Malignant neoplasm of pancreas*, not prostate. `32798002` is *Parkinsonism*, not Parkinson's disease. Those queries could only ever have returned zero.
